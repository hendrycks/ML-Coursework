...
class ClassifierTrainer(object):
  ...
  def train(self, X, y, X_val, y_val, 
            ...
            update='momentum', hidden_size = 50, sample_batches=True,
    ...
    for it in xrange(num_iters):
      ...
      for p in model:
        ...
        if update == 'sgd':
          ...
          #                      END OF YOUR CODE                             #
          #####################################################################
        elif update == 'rmsprop+momentum':
          decay_rate = 0.99 # you could also make this an option
          if not p in self.step_cache: 
            self.step_cache[p] = np.array([np.zeros(grads[p].shape), np.zeros(grads[p].shape)]) #rms, momentum
          self.step_cache[p][0] = decay_rate * self.step_cache[p][0] + (1 - decay_rate) * (grads[p] ** 2)
          self.step_cache[p][1] = momentum * self.step_cache[p][1] - \
            learning_rate * grads[p] / np.sqrt(self.step_cache[p][0] + 1e-8)
          dx = self.step_cache[p][1]
        else:
      ...
      if first_it or epoch_end or acc_check:
        ...
        if verbose:
          print ('Finished epoch %d / %d: cost %f, train: %f, val %f, lr %e, reg %e'
                 % (epoch, num_epochs, cost, train_acc, val_acc, learning_rate, reg))

    print 'val %f, lr %e, reg %e, momentum %.3f, hidden_size %i, epochs %i, train %f' % (best_val_acc, learning_rate, reg, momentum, hidden_size, num_epochs, train_acc)
    # return the best model and the training history statistics
    return best_model, loss_history, train_acc_history, val_acc_history

 # hidden size passed to function for printing purposes
