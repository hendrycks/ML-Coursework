...
class ClassifierTrainer(object):
  ...
  def train(self, X, y, X_val, y_val, 
            model, loss_function, 
            reg=0.0, dropout=1.0,
            learning_rate=1e-2, momentum=0.9, learning_rate_decay=0.95,
    ...
    for it in xrange(num_iters):
      ...
      for p in model:
        ...
        if update == 'sgd':
          ...
          decay_rate = 0.99 # you could also make this an option
          if not p in self.step_cache: 
            self.step_cache[p] = [np.zeros(grads[p].shape),np.zeros(grads[p].shape)] #rms 0, momentum 1
          self.step_cache[p][0] = self.step_cache[p][0] * decay_rate + (1 - decay_rate) * grads[p] ** 2
    ...
    print 'Done. val %f, lr %f, dropout %.2f, reg %.3f, num_epochs %d, train %f' % (val_acc, learning_rate, dropout, reg, num_epochs, train_acc)
