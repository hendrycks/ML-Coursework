...
def random_crops(X, crop_shape):
  ...
  # find farthest placement of crop window
  max_x_pos = W - WW
  max_y_pos = H - HH
  ...
  for n in xrange(N):
    x_pos = np.random.randint(max_x_pos)
    y_pos = np.random.randint(max_y_pos)
    out[n,:] = X[n,:, x_pos:x_pos + WW, y_pos:y_pos + HH]
...
def random_contrast(X, scale=(0.8, 1.2)):
  ...
  for n in xrange(N):
    out[n,:] = X[n,:] * (low + np.random.rand() * (high - low))
...
def random_tint(X, scale=(-10, 10)):
  ...
  for n in xrange(N):
    for c in xrange(C):
      out[n,c] = X[n,c] + (  low + (high - low) * np.random.rand()  )
